...
int initOpenGL(screen_context_t screen_cxt) {
	...
	font = bbutil_load_font(
			"app/native/Elsa-Regular.otf", 9, dpi);
	if (!font) {
		return EXIT_FAILURE;
	}
	...
	float tex_x, tex_y;
	if (EXIT_SUCCESS
			!= bbutil_load_texture("app/native/bg_hochformat.png", NULL, NULL, &tex_x,
					&tex_y, &background)) {
...
void render() {
	...
	while (pp != points.end()) {
		...
		if (pp->visible) {
			// draw touchpoint
			glPushMatrix();
				glTranslatef(pp->getX(), (1280) - pp->getY(), 0);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
			...
			glPushMatrix();
				...
				glTranslatef(pp->getX()+50,1280-pp->getY()+50,0);
				glPushMatrix();
